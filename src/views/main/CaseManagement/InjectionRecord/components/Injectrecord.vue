        ...
        <template v-for="(item, index) in labelList">
          <el-table-column :prop="item.name" :label="item.label" :key="index">
            <template slot-scope="scope">
              ...
              <template v-else-if="item.name=='injectStatus'">
                <span v-if="scope.row[item.name]=='0'">待注射</span>
                <span v-if="scope.row[item.name]=='1'">待确认</span>
                <span v-if="scope.row[item.name]=='2'">已确认</span>
                <span v-if="scope.row[item.name]=='3'">已逾期</span>
              </template>
...
import detailInject from "./detailInject";
  ...
  methods: {
    ...
    _exportData(form) {
      ...
      axios({
        ...
        method: "post",
        url: `${process.env.BASE_API}/excelDownload/getInjectionRecordListExcel`,
        data: form,
